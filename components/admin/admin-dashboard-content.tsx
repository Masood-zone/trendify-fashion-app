...
export function AdminDashboardContent() {
  ...
  if (query.isError || !query.data)
    return (
      <ErrorPanel
        message={query.error?.message || "Dashboard could not be loaded"}
        retry={() => query.refetch()}
      />
    )
  ...
  return (
    <>
      <AdminPageHeader
        title="Dashboard"
        description="A live view of store performance, fulfilment, and inventory health."
      />
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
        <MetricCard
          label="Verified Revenue"
          value={formatPesewas(data.summary.revenuePesewas, true)}
          icon="payments"
        ...
        <MetricCard
          ...
          icon="group"
          change={data.changes.customers}
        />
        <MetricCard
          label="Products in Stock"
          value={data.summary.productsInStock.toLocaleString()}
          icon="inventory_2"
        />
        <MetricCard
        ...
        <MetricCard
          ...
          value={data.summary.lowStockProducts.toLocaleString()}
          icon="warning"
          danger={data.summary.lowStockProducts > 0}
        />
      </div>
        ...
        <AdminCard className="p-6">
          <div className="mb-5">
            ...
            <p className="text-sm text-muted-foreground">
              Server-verified Paystack payments over the last 12 months
            </p>
          </div>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.monthlyRevenue}>
                ...
                <YAxis
                  tickFormatter={(value) =>
                    `${Math.round(Number(value) / 10000)}k`
                  }
                  tickLine={false}
                  axisLine={false}
                  width={45}
        ...
        <AdminCard className="p-6">
          ...
          {orderChart.length ? (
            ...
            <EmptyState
              ...
              description="Order status distribution will appear after orders are placed."
            />
          )}
        </AdminCard>
      </div>
      <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_2fr]">
          ...
          <div className="mt-5 space-y-5">
            {data.salesByCategory.length ? (
              data.salesByCategory.map((item) => {
                const max = data.salesByCategory[0]?.valuePesewas || 1
                return (
                  <div key={item.name}>
                    <div className="mb-2 flex justify-between gap-3 text-sm">
                      <span>{item.name}</span>
                      <strong>{formatPesewas(item.valuePesewas, true)}</strong>
                    </div>
                    <div className="h-2 bg-surface-container">
                      <div
                        className="h-full bg-primary"
                        style={{
                          width: `${Math.max(4, (item.valuePesewas / max) * 100)}%`,
                        }}
                      />
                    </div>
                  </div>
        ...
        <AdminCard className="heritage-pattern p-6">
          <h2 className="font-heading text-xl font-semibold">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            ...
            <Button
              render={<Link href="/admin/discounts/new" />}
              variant="outline"
              className="justify-start"
            >
              <MaterialSymbol icon="sell" />
              Create Discount
            </Button>
            <Button
          ...
          {data.summary.lowStockProducts > 0 && (
            <div className="bg-error-container mt-6 flex gap-3 border border-error/20 p-4 text-sm text-error">
              <MaterialSymbol icon="campaign" />
              <span>
                Inventory notice: {data.summary.lowStockProducts} product
                {data.summary.lowStockProducts === 1 ? " is" : "s are"} at or
          ...
          {data.lowStock.length ? (
            <div>
              {data.lowStock.map((product) => (
                ...
                >
                  <div className="grid size-12 place-items-center bg-surface-container text-muted-foreground">
                    {product.imageUrl ? (
                      // Cloudinary URLs are already transformed for these compact admin thumbnails.
            ...
            <EmptyState
              ...
              description="No products are currently below their configured thresholds."
            />
          )}
        </AdminCard>
      </div>
